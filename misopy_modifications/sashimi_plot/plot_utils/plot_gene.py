...
def coordinates_from_BedTracks(chrom, tx_start, tx_end, additional_tracks):
    dTracks = []
    track_names = []
    for t in additional_tracks:
        track_names.append(
            os.path.basename( t.strip() )
        )
        track_line = []
        with open( t, 'r') as f:
            for line in f:
                if line.startswith('track') \
                    or not line.startswith( chrom ):
                    continue
                sChrom, sStart, sEnd, sName = line.strip().split('\t')[:4]
                
                # is the event inside the tx_start and tx_end boundaries?
                if tx_start <= int(sStart) <= tx_end:
                    track_line.append([ int(sStart), int(sEnd) ])
#        if len(track_line):
        dTracks.append(track_line)
    print "*" * 60
    print tx_start, tx_end
    print "*" * 60
    print dTracks, track_names
    print "*" * 60
    
    return dTracks, track_names
...
#                 bar_posterior=False):
    ...
    additional_tracks = settings["additional_tracks"]
    ...
    tx_start, tx_end, exon_starts, exon_ends, gene_obj, mRNAs, strand, chrom, mRNA_ids = \
...
#        curr_ax.grid(True)
    ...
    # Draw extra tracks
    # @author: Wai Yi Leung
    ax = subplot2grid((nfiles + 3, gene_posterior_ratio), (nfiles + 0, 0), colspan=gene_posterior_ratio - 1, rowspan=1)
    add_tracks, track_names = coordinates_from_BedTracks(chrom, tx_start, tx_end, additional_tracks)
    plot_mRNAs(tx_start, add_tracks, None, graphcoords, reverse_minus, mRNA_names=track_names)
    ...
    plot_mRNAs(tx_start, mRNAs, strand, graphcoords, reverse_minus, mRNA_ids)
    subplots_adjust(hspace=.1, wspace=.7)
...
def plot_mRNAs(tx_start, mRNAs, strand, graphcoords, reverse_minus, mRNA_names=[]):
    """
    ...
    for mMRA_no, mRNA in enumerate(mRNAs):
        for s, e in mRNA:
            s = s - tx_start
            e = e - tx_start
        ...
        # Add label
        if len(mRNA_names) >= mMRA_no+1:
            text(5, yloc+0.25, mRNA_names[mMRA_no],fontsize=6)
